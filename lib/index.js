...
//#endregion
//#region src/suppress.ts
/** 重复调用提醒注入消息使用的事件 source kind. */
const REPEAT_REMINDER_SOURCE_KIND = "repeat-tool-reminder";
/**
* 判断一条消息是否来自重复调用提醒.
...
function apply(ctx, config) {
	...
	ctx.logger.info("%s: settings bound (defaultLimitMs=%d ms, hardLimitMs=%d ms, allowEscalation=%s, requireExplicitJobWaitMs=%s)", PLUGIN_ID, initial.defaultLimitMs, initial.hardLimitMs, String(initial.allowEscalation), String(initial.requireExplicitJobWaitMs));
		...
		promptCtx.systemPrompt.context({
			...
			text: (context) => {
				...
				return describeWaitBudget({
					settings: current,
					sessionLimitMs: sessionLimits.limitOf(session, current.defaultLimitMs, current.hardLimitMs),
					raised
				});
			}
		});
